    ...
    try {
      ...
      for (i = 0; i < payload.length; i++) {
        ...
        if (~GITHUB_ACCEPTED_CREATORS.indexOf(author)) {
          var newVisibleLabels = addLabelsAndGetSanitzed(issue.labels);
          var newIssue = {
            number: issue.number,
            title: issue.title,
            body: issue.body,
          ...
          var found = allIssues.some((el) => el.number === newIssue.number);
...
var ready = function () {
  ...
  for (k = 0; k < elms.length; k++) {
    ...
    if (id === null) {
      var prefix = elms[k].getAttribute(dataLabelName).split(" ").join("");
